...
Consider 
\begin{align*}
\int_\mathbb{R} |x| dF &= \int_0^\infty x\lambda e^{-\lambda x} dx \\
...
\end{theorem}
\begin{proof}
Let $\mu=\mathbb{E}(X)$. Then $\mathbb{E}(a + bX) = a + b\mu$.
So
\begin{align*}
...
\begin{equation}
\mathbb{E}(e^{t(X-\mu)})\leq e^\frac{t^2\xi^2}{2}
\end{equation}
...
Then
\begin{equation}
Pr(|X-\mu|\geq t)\leq\frac{Var(X)}{t^2}
\end{equation}
\end{theorem}
\begin{proof}
Let $U=(X-\mu)^2$.
Then $U$ is non-negative and $\mathbb{E}(U)=Var(X)$.
So the Markov bound gives $Pr((X-\mu)^2\geq t^2)\leq \frac{Var(X)}{t^2}$.
\end{proof}

\begin{theorem}
\textbf{Chernoff Bound:}\label{chernoff}
Let $X$ be a random variable with MGF $M_X$.
Then
\begin{equation}
